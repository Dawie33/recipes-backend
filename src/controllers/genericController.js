...
const genericController = {
    ...
    getAll: (dataAccessLayer) => async (req, res) => {
      try {
        const items = await dataAccessLayer.getAll(); // Appel générique de la couche d'accès aux données
  ...
  getById: (dataAccessLayer) => async (req, res) => {
    ...
    create: (dataAccessLayer) => async (req, res) => {
      try {
        const newItem = await dataAccessLayer.create(req.body); // Appel générique de la couche d'accès aux données
    ...
    delete: (dataAccessLayer) => async (req, res) => {
      try {
        const id = req.params.id;
        const deletedItem = await dataAccessLayer.delete(id); // Appel générique de la couche d'accès aux données
        if (!deletedItem) {
          return res.status(404).json({ error: 'Élément non trouvé' });
        }
